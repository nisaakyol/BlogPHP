...
/**
 * Datei: admin/posts/postActions.php
 * Zweck: Handle "submit" (zur Prüfung einreichen) und "delete" (löschen)
 * Aufruf: ausschließlich per POST + CSRF
 */
...
// Post laden (für Besitz-/Statusprüfung)
...
switch ($action) {
  case 'submit':
    // nur Besitzer; Admins reichen nicht selbst ein
    if (!$isOwner || $isAdmin) {
      $_SESSION['message'] = 'Nicht erlaubt.';
      $_SESSION['type']    = 'error';
      break;
    }
    // nur aus Entwürfen oder nach Ablehnung
  ...
  case 'delete':
    // Löschen: Besitzer oder Admin
    if (!$isOwner && !$isAdmin) {
      $_SESSION['message'] = 'Nicht erlaubt.';
      $_SESSION['type']    = 'error';
      break;
    }
    // Reihenfolge: Comments → post_topic (falls vorhanden) → Post
    try {
