...
{
    /**
     * Liefert 0..100. Berücksichtigt:
     * - Titel-Exact / Prefix / Token-Fuzzy (Levenshtein)
     * - Body-Containment grob
     * - Recency-Boost
     */
    public static function score(string $query, string $title, string $body, ?string $createdAt = null): float
    {
        ...
        if (str_contains($t, $q))    $score += 25;                 // Substring Titel

        // 2) TOKEN-LEVENSHSTEIN im Titel (einzelne Wörter vergleichen)
        ...
        if ($createdAt) {
            $ageDays = max(0, (time() - strtotime($createdAt)) / 86400);
    ...
    /**
     * Vergleicht Query gegen die besten Titel-Tokens.
     * Kurze Queries (<=4) erhalten tolerantere Bewertung.
     */
